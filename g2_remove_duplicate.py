...
for bin_path in config.G1_PORGRAM_ARR:
    # config.FEA_DIR + str(os.sep) + bin_path
    path = glob.glob(config.FEA_DIR + str(os.sep) + bin_path + os.sep + "*" + os.sep + "*")
    for paths in path:
        print(paths)
        temppath = paths + os.sep + 'temp'
        if os.path.exists(temppath):  # 有重复项，在temp文件夹
            funset = set()
            ...
            while 1:
                ...
                if not line:
                    ...
                    if((bin_path == "openssl") and (funname in config.G1_CVE_OPENSSL_FUN_LIST.keys())):
                        if funname in funset:
                            srcfile_pre = paths + os.sep + "temp" + os.sep + funname
                        else:
                            srcfile_pre = paths + os.sep + funname
                        dstfile_pre = config.CVE_FEATURE_DIR + os.sep + config.G1_CVE_OPENSSL_FUN_LIST.get(
