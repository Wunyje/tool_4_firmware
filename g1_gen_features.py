...
def calConstantNumber(ea):
	...
	while i <= 1:
		if (idc.GetOpType(ea,i ) == 5):  # ida pro 函数
			addr = idc.GetOperandValue(ea, i)
			if (idc.SegName(addr) == '.rodata') and (idc.GetType(addr) == 'char[]') and (i == 1):  # rodata = read-only data
...
# 分别按Gemini和Vulseeker提取基本块特征，前者7个，后者8个
...
def sav_arch_info(target_firmware):
	...
	if( processor_name == "metapc"):
		...
		processor_name = 'msp430'
	elif( processor_name == "mipsl" or processor_name == "mipsb"):
		processor_name = 'mips32'
	elif( processor_name == "PPC"):
		processor_name = 'PPC'
	elif( processor_name == "octeonb"):
		processor_name = 'mips64'
	else:
		processor_name = 'undefine'
		
	arch_info_path = os.path.join(config.ORIGIN_DIR, 'search_program', target_firmware)
	open(arch_info_path + os.sep + processor_name + '.txt', 'w')
	return 0
...
def main():
	...
	fea_path_origin = ""
	if len(idc.ARGV) < 1:  # IDC:IDA的脚本语言
		fea_path_origin=config.FEA_DIR
		bin_path = config.ORIGIN_DIR
	...
	sav_arch_info(version)
	
	print("Directory path:" , fea_path_origin)
	function_list_file = fea_path_origin + os.sep + "functions_list_fea.csv"
